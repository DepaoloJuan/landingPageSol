...
- **Auth:** `@react-oauth/google` — **solo en `fidelizacion-front`**, primera dependencia de auth que tiene este repo. Convive con un login/registro manual por email+contraseña (sin librería extra, contra el backend propio)
...
src/
 ...
 │         ├── Dashboard.tsx           # orquesta TarjetaSellos + PremioCard[] + Historial; saludo "¡Bienvenida, {nombre}!"
 ...
 │    └── fidelidad/
 │         ├── FidelidadPage.tsx        # orquestador con máquina de estados del portal + toggle Google/email
...
- `POST /api/fidelidad/registro` → `{ token, requiere_telefono, estado_vinculacion? }` (alta manual: nombre + email + password + teléfono en un solo request, sin paso de vinculación de teléfono aparte)
...
- **Login dual (Google + email/contraseña), no solo Google:** el flujo original ofrecía únicamente login con Google, lo que dejaba afuera a clientas sin cuenta de Gmail (Yahoo, iCloud, Hotmail, etc.). Se agregó un link secundario ("¿No tenés Gmail? Ingresá con tu email") que despliega un toggle interno en `FidelidadPage` con tres vistas (login, registro, olvidé mi contraseña) sin salir de `/mi-fidelidad`. El registro manual pide el teléfono en el mismo paso que el resto de los datos (a diferencia del flujo de Google, que lo pide después vía `TelefonoForm`), porque no hay otro momento post-login natural donde pedirlo.
- **Botón de cerrar sesión sacado de `Dashboard.tsx`, fijado en `FidelidadPage.tsx`:** antes vivía dentro del contenido del dashboard; se movió a un botón `position: fixed` arriba a la derecha de toda la pantalla, fuera del `Container` centrado. Separa una acción de "chrome" de la página (disponible en cualquier momento que haya sesión activa) del contenido en sí (que ahora arranca directo con el saludo a la clienta), para que no compitan visualmente.
...
- **En desarrollo, rama `fidelizacion-front` (NO mergeada a `main`, sin deployar):** portal de fidelización completo a nivel código — router, login dual (Google o email/contraseña), registro manual, reseteo de contraseña por mail, alta de teléfono, tarjeta de sellos, revelado de premios, historial de turnos (colapsado a los últimos 3, con opción de ver todo). PR abierto: https://github.com/DepaoloJuan/landingPageSol/pull/new/fidelizacion-front. Depende del backend de **Sol Admin, rama `fidelizacion`** (repo separado) para funcionar de punta a punta — ese backend agregó los endpoints `registro`, `login`, `olvide-password` y `resetear-password`, más envío de mails con Resend (plantilla HTML con la marca) para el flujo de reseteo.
  - **Prerequisito externo pendiente:** falta configurar `VITE_GOOGLE_CLIENT_ID` (mismo OAuth Client ID de Google Cloud que necesita el backend) — sin eso el botón de login con Google no funciona. Variable agregada vacía en `.env.local` (no trackeado). El login/registro por email no depende de esto.
