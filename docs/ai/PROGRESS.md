...
**Qué se hizo:**
- Se mergeó `fidelizacion-front` a `main` (PR #1, merge commit `535e732`), sumando por encima de las dos rondas ya registradas los commits: `dbc2bb3` (tarjetas anteriores en el dashboard), `879a85b` (fix de navegación y ajustes visuales), `20c4ad4` (carrusel coverflow), `fa58d4b` (fix responsive mobile), `9d5986f` (chore de gitignore/tracking) y `a61e744` (PWA instalable).
- **Tarjetas anteriores** (`TarjetasAnteriores.tsx`, luego reemplazado por `TarjetasCarousel.tsx`): lista de tarjetas de fidelización ya completadas con su premio, usando el mismo patrón "últimas 3 + ver todas" de `Historial.tsx`. `fidelidadApi.ts` suma `getTarjetasAnteriores()`, y `Premio.sello_numero` deja de estar limitado a 5|10 (ahora configurable desde el admin de Sol Admin).
- **Carrusel coverflow** (`TarjetasCarousel.tsx`, 186 líneas): reemplaza el listado simple de tarjetas anteriores por un carrusel visual tipo coverflow.
- **Fix responsive mobile**: reescritura de `TarjetaSellos.tsx` y ajustes en `TarjetasCarousel.tsx` para que la tarjeta de sellos se vea bien en pantallas chicas.
- **Navegación y ajustes visuales**: link "Mi Fidelidad" en el nav del header, botón de volver al sitio desde el portal y desde la pantalla de reseteo de contraseña; se restauraron `Hero.tsx`, `tailwind.config.js` e `index.css` al estado real de producción (habían quedado desincronizados del WIP viejo).
- **Chore de limpieza**: `.gitignore` ahora ignora `.env`/`.env.*` (con excepción de `.env.example`); se destrackearon `node_modules/.package-lock.json`, `node_modules/.vite/deps/_metadata.json` y `repo_completo.txt`, que habían quedado mal trackeados.
- **PWA instalable "Mi Fidelidad"** (`a61e744`): `public/manifest-fidelidad.json` (scope `/mi-fidelidad/`, standalone, iconos `pwa-192/512`, maskable) + `public/sw-fidelidad.js` (service worker con cache stale-while-revalidate para el shell de la app, excluyendo explícitamente todo lo que empiece con `/api/` para que los datos de sellos/premios nunca se sirvan de caché). El hook `src/lib/usePwaFidelidad.ts` registra el manifest y el service worker solo mientras la clienta navega dentro de `/mi-fidelidad`, y los desregistra al salir — así el prompt de "Instalar app" de Chrome no aparece en el resto de la landing pública. Aclaración: a pesar de que en su momento se pensó como funcionalidad de "push", esta implementación NO incluye push notifications reales, solo instalabilidad (PWA "add to home screen").
- Junto con la PWA se agregó `PremioModal.tsx` (modal de revelado de premio con animación de spinner + reveal, reemplazando el flip anterior de `PremioCard.tsx`) y configuración de Firebase Hosting (`firebase.json`, `.firebaserc`, proyecto `sol-cantero`) trackeada en git por primera vez.
- **Deploy real a producción**: la rama no quedó solo mergeada en git — se deployó a Firebase Hosting hoy mismo. Confirmado con `firebase hosting:channel:list`: release del 2026-08-06 21:00:25 sirviendo tráfico en `sol-cantero.web.app`. El portal de fidelización (login Google + email/contraseña, tarjeta de sellos, carrusel de tarjetas anteriores, revelado de premio, canje en persona, reseteo de contraseña vía mail, PWA instalable) está en vivo.

**Archivos tocados:** `src/components/fidelidad/TarjetasAnteriores.tsx` (agregado y luego borrado), `src/components/fidelidad/TarjetasCarousel.tsx` (nuevo), `src/components/fidelidad/TarjetaSellos.tsx`, `src/components/fidelidad/PremioModal.tsx` (nuevo), `src/components/fidelidad/PremioCard.tsx`, `src/components/fidelidad/Dashboard.tsx`, `src/components/fidelidad/Historial.tsx`, `src/components/layout/Header.tsx`, `src/components/layout/Footer.tsx`, `src/components/sections/Hero.tsx`, `src/pages/fidelidad/FidelidadPage.tsx`, `src/pages/fidelidad/ResetearPasswordPage.tsx`, `src/lib/fidelidadApi.ts`, `src/lib/constants.ts`, `src/lib/usePwaFidelidad.ts` (nuevo), `public/manifest-fidelidad.json` (nuevo), `public/sw-fidelidad.js` (nuevo), `public/pwa-192.png`/`pwa-512.png`/`pwa-maskable-512.png`/`apple-touch-icon.png` (nuevos), `firebase.json` (nuevo), `.firebaserc` (nuevo), `.gitignore`, `tailwind.config.js`, `src/index.css`.

**Pendiente / a revisar:**
- Local `main` en el checkout usado para esta bitácora estaba 9 commits detrás de `origin/main` (no se hizo `git pull` ni ningún otro cambio de estado de git desde este agente, solo se escribió este archivo).
- El WIP viejo que venía de entradas anteriores (config previa de Firebase distinta a la que trajo `a61e744`, fotos, `curso_soft_gel_v2.html`, ajustes de estilos sueltos en `About/Courses/Gallery/Services/Testimonials/IntroScreen`) sigue sin commitear en el working tree — no se tocó en esta ronda.
- No se verificó en esta entrada el flujo de canje de premios en persona ni el de PWA end-to-end contra dispositivos reales, más allá de la lectura del código.
